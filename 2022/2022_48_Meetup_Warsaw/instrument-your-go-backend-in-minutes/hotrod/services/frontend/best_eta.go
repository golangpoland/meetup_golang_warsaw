...
func newBestETA(tp trace.TracerProvider, logger log.Factory, options ConfigOptions) *bestETA {
	return &bestETA{
...
func (eta *bestETA) Get(ctx context.Context, customerID string) (*Response, error) {
	...
	if span := trace.SpanFromContext(ctx); span != nil {
		m, err := baggage.NewMember("customer_id", customer.ID)
		if err != nil {
			panic(fmt.Errorf("failed to create new member: %w", err))
		}

		b, err := baggage.New(m)
		if err != nil {
			panic(fmt.Errorf("failed to create new baggage: %w", err))
		}
